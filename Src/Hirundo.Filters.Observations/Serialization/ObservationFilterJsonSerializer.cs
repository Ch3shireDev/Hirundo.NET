...
public class ObservationFilterJsonSerializer
{
    ...
    private static string SerializeFilterWithType(IObservationFilter filter, string type)
    {
    ...
    public IObservationFilter Deserialize(string json)
    {
        ...
        return type switch
        {
            "IsEqual" => JsonConvert.DeserializeObject<IsEqualValueFilter>(json) ?? throw new ObservationFilterSerializationException("Błąd deserializacji warunku obserwacji typu IsEqual."),
            "IsInTimeBlock" => JsonConvert.DeserializeObject<IsInTimeBlockFilter>(json) ??
